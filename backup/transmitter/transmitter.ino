...
void loop() {
  ...
  // ---- Send UDP broadcast every SEND_INTERVAL_MS (kept for compatibility / heartbeat data) ----
  if (now - lastSendTime >= SEND_INTERVAL_MS) {
    lastSendTime = now;
    sendUdpPacket();
  }

  // ---- Echo back any unicast packet received (e.g. pings from the receiver) ----
  // This creates a steady unicast request/response exchange, which is
  // transmitted at HT rates (unlike broadcast/beacon frames) and reliably
  // carries the LTF fields needed for continuous CSI extraction on the
  // receiver's end.
  ...
  if (packetSize > 0) {
    ...
    // Must fully consume the incoming datagram, or the UDP socket can stall
    // and stop accepting further packets after just one exchange.
    uint8_t discardBuf[64];
    int len = udp.read(discardBuf, sizeof(discardBuf));
    (void)len; // Content doesn't matter, we just need to drain the buffer
